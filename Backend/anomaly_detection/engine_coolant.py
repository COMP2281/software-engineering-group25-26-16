...
class EngineCoolantClassifier():
    ...
    def __init__(self, data_path: str) -> None:
        self._knn, self._threshold = self.init_model(data_path)
    ...
    def generate_warnings(self, filepath) -> list[BaseWarning]:
        ...
        print(f"Generating engine coolant warnings for file: {filepath}")
        ...
        print("================================================================================")
        print(self._train_mean)
        print(self._train_std)
        print("================================================================================")
        predictions, _ = self.detect_anomalies(self._knn, self._threshold, windows)
    ...
    def windows_to_matrix(self, windows):
    ...
    def normalise_train(self, X):
        # compute mean and std from training data
        self._train_mean = np.mean(X, axis=0)
        self._train_std = np.std(X, axis=0)
    ...
    def normalise_test(self, X):
        # apply the same scaling used for training
        return (X - self._train_mean) / self._train_std


    def train_oneclass_knn(self, normal_windows):
        # convert windows into feature vectors
        X_train = self.windows_to_matrix(normal_windows)

        # scale the features so they are comparable
        X_train = self.normalise_train(X_train)
    ...
    def detect_anomalies(self, model, threshold, test_windows):
        # convert test windows into features
        X_test = self.windows_to_matrix(test_windows)

        # scale test data using training statistics
        X_test = self.normalise_test(X_test)
